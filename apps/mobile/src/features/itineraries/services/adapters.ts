...
import { tripPreferencesSchema, type GeneratedItinerary, type TripPreferences } from '@saraya/contracts';

import { getApiBaseUrl } from '@/core/config';

import type { ItineraryGateway, PendingItineraryStore, PremiumAccessGateway } from '../gateways';

const PENDING_KEY = '@saraya/pending-itinerary';

export class PremiumIntegrationUnavailableError extends Error {
  constructor() {
    super('Premium access is unavailable until the RevenueCat integration is configured.');
    this.name = 'PremiumIntegrationUnavailableError';
  }
}
...
class UnavailablePremiumAccessGateway implements PremiumAccessGateway {
  async getEntitlement(): Promise<never> {
    throw new PremiumIntegrationUnavailableError();
  }

  async requestPurchase(): Promise<never> {
    throw new PremiumIntegrationUnavailableError();
  }
}
...
export const premiumAccessGateway: PremiumAccessGateway = new UnavailablePremiumAccessGateway();
