...
export function ItineraryPlannerScreen() {
  ...
  if (status === 'generating') {
    return (
      <Screen contentContainerStyle={styles.centeredScreen}>
        <Mascot mood="star" size={150} />
        <Text accessibilityRole="header" style={styles.centerTitle}>Building your {destinationName} itinerary</Text>
        <Text style={styles.centerBody}>Matching stops to your {pace.toLowerCase()} pace and {budget.toLowerCase()} budget.</Text>
        {['Preferences understood', 'Travel times checked', 'Food and stays matched', 'Day-by-day plan'].map((step, index) => (
        ...
        <StatusPanel message="Cancel safely and return to the choices you saved." title="Your preferences stay saved" />
        <Button icon={X} label="Cancel generation" onPress={() => abortRef.current?.abort()} variant="secondary" />
      </Screen>
    );
  }
      ...
      <Screen>
        ...
        <View style={styles.dayTabs}>
          {itinerary.days.map((item) => (
            <Chip key={item.dayNumber} label={`Day ${item.dayNumber}`} onPress={() => setActiveDay(item.dayNumber)} selected={activeDay === item.dayNumber} />
          ))}
        </View>
        <Text style={styles.dayTitle}>{day?.title}</Text>
        {day?.stops.map((stop, index) => {
          const Icon = { transport: Bus, activity: Waves, meal: Utensils, stay: BedDouble }[stop.kind];
          return (
    ...
    <Screen>
      ...
      <View style={styles.headerRow}>
        <View style={styles.headerCopy}>
          ...
          <Text style={styles.subtitle}>{destinationName} · choices are saved before premium gating</Text>
        </View>
      ...
      <PreferenceGroup label="Trip length" onChange={setDurationDays} options={durations} value={durationDays} />
      <PreferenceGroup label="Budget" onChange={setBudget} options={budgets} value={budget} />
      <View style={styles.formGroup}>
        <Text style={styles.label}>Interests</Text>
        <View style={styles.wrap}>{interests.map((interest) => <Chip key={interest} label={interest} onPress={() => toggleInterest(interest)} selected={selectedInterests.includes(interest)} />)}</View>
      </View>
...
const styles = StyleSheet.create({
  ...
  formGroup: { gap: spacing.sm }, label: { color: colors.navy, fontFamily: type.black, fontSize: 15 },
  input: { minHeight: 52, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, color: colors.navy, fontFamily: type.medium, fontSize: 16, paddingHorizontal: spacing.lg },
  ...
  centerBody: { color: colors.muted, fontFamily: type.medium, fontSize: 15, lineHeight: 23, textAlign: 'center' },
  progressRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, width: '100%' },
  progressDot: { width: 36, height: 36, borderRadius: 18, borderWidth: 2, borderColor: colors.border, backgroundColor: colors.surface, alignItems: 'center', justifyContent: 'center' },
  ...
  stopRow: { flexDirection: 'row', minHeight: 94 }, stopRail: { width: 52, alignItems: 'center' },
  stopIcon: { width: 42, height: 42, borderRadius: 21, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.yellowSoft },
  railLine: { flex: 1, width: 2, backgroundColor: colors.border }, stopCopy: { flex: 1, paddingBottom: spacing.xl, gap: 2 },
  stopTime: { color: colors.muted, fontFamily: type.bold, fontSize: 11 }, stopTitle: { color: colors.navy, fontFamily: type.black, fontSize: 15 },
  stopDetail: { color: colors.muted, fontFamily: type.medium, fontSize: 13, lineHeight: 19 },
  actions: { gap: spacing.sm }, primaryAction: { flex: 1 },
});
